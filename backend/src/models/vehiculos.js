...
Vehiculo.obtenerVehiculos = (callback) => {
  poolc.query("SELECT V.ID_VEHICULO, A.NOMBRE_AGENCIA, V.TIPO_VEHICULO, V.IMAGEN_VEHICULO, V.KILOMETRAJE_VEHICULO, V.NOMBRE_VEHICULO, V.MODELO_VEHICULO, V.TRANSMISION_VEHICULO, V.RATING_VEHICULO, V.DESCRIPCION_VEHICULO, V.PRECIO_VEHICULO, V.DISPONIBILIDAD_VEHICULO FROM VEHICULO V JOIN AGENCIA A ON V.ID_AGENCIA = A.ID_AGENCIA", (err, results) => {
    if (err) {
      callback(err, null);
    } else {
